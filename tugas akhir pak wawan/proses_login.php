...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $user_input = $_POST['email'] ?? '';  // Input bisa username atau email
    ...
    // Query mencari user berdasarkan email ATAU username
    ...
    if ($stmt->num_rows === 1) {
        ...
        if ($role === 'admin') {
            // Kalau admin passwordnya plain text? Kalau iya, cek begini:
            if ($password === $password_db) {
                $valid_login = true;
            }
        } else {
            // Untuk user biasa pakai password_verify
